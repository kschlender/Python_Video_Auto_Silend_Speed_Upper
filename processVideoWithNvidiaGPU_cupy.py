...
for i in tqdm(range(num_chunks)):
    ...
    rates = cp.array([4.0 if muted_audio_mask[j] else 1.0 for j in range(len(muted_audio_mask))])
    ...
    frames = cp.array([chunk.get_frame(t) for t in range(len(rates))])
    frames = cp.asnumpy(frames)
    frames *= rates.reshape(-1, 1, 1, 1)
    frames = cp.array(frames)
    video_clip = concatenate_videoclips([VideoFileClip(None, fps=chunk.fps, ismask=False, duration=1.0 / chunk.fps, ndarray=cp.asnumpy(frames[l])) for l in range(len(rates))], method="compose")
